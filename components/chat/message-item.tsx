...
export function MessageItem({ message, isNew = false }: MessageItemProps) {
  ...
  // Determine if typing effect should be enabled
  const shouldUseTypingEffect = !isUser && isNew && typeof message.content === "string" && message.content.length > 0;
  
  // Get the text content for typing effect
  const textContent = typeof message.content === "string" ? message.content : "";
  const [displayedText, isTypingComplete] = useTypingEffect(textContent, {
    enabled: shouldUseTypingEffect,
    speed: 25 // Slower speed for more visible effect (was 15)
  });
  
  // Debug logging
  useEffect(() => {
    if (shouldUseTypingEffect) {
      logger.info(`[MessageItem] Typing effect enabled for message ${message.id}, isNew: ${isNew}, content length: ${textContent.length}`);
    }
  }, [shouldUseTypingEffect, message.id, isNew, textContent.length]);
  ...
  const renderContent = () => {
          ...
          {typeof message.content === "string" && message.content && (
            <div className="prose prose-sm dark:prose-invert max-w-none break-words">
              <ReactMarkdown remarkPlugins={[remarkGfm]}>
                {shouldUseTypingEffect ? displayedText : message.content}
              </ReactMarkdown>
            </div>
          )}
        </div>
    ...
    if (typeof message.content === "string") {
      // Use displayedText when typing effect is active, otherwise use original content
      const contentToDisplay = shouldUseTypingEffect ? displayedText : message.content;
      ...
      return (
        <div className="prose prose-sm dark:prose-invert max-w-none break-words">
          ...
          {shouldUseTypingEffect && !isTypingComplete && (
            <span className="inline-block w-1 h-5 bg-primary animate-pulse ml-1 align-text-bottom" />
          )}
        </div>
      );
    }
