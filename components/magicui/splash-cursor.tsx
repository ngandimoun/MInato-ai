...
class PointerInstance implements Pointer {
  ...
  constructor() { this.id = -1; this.texcoordX = 0; this.texcoordY = 0; this.prevTexcoordX = 0; this.prevTexcoordY = 0; this.deltaX = 0; this.deltaY = 0; this.down = false; this.moved = false; this.color = { r: 0, g: 0, b: 0 }; }
}
interface SupportedFormat { internalFormat: number; format: number; }
interface WebGLExtensions { formatRGBA: SupportedFormat | null; formatRG: SupportedFormat | null; formatR: SupportedFormat | null; halfFloatTexType: number | undefined; supportLinearFiltering: OES_texture_float_linear | null; }
interface FBO { texture: WebGLTexture | null; fbo: WebGLFramebuffer | null; width: number; height: number; texelSizeX: number; texelSizeY: number; attach: (id: number) => number; }
interface DoubleFBO { width: number; height: number; texelSizeX: number; texelSizeY: number; read: FBO; write: FBO; swap: () => void; }
function SplashCursor({
  SIM_RESOLUTION = 128, DYE_RESOLUTION = 1440, DENSITY_DISSIPATION = 3.5, VELOCITY_DISSIPATION = 2, PRESSURE = 0.1,
  PRESSURE_ITERATIONS = 20, CURL = 3, SPLAT_RADIUS = 0.2, SPLAT_FORCE = 6000, SHADING = true, COLOR_UPDATE_SPEED = 10,
  ...
  useEffect(() => {
    ...
    let isActive = true;
    const config = { SIM_RESOLUTION, DYE_RESOLUTION, DENSITY_DISSIPATION, VELOCITY_DISSIPATION, PRESSURE, PRESSURE_ITERATIONS, CURL, SPLAT_RADIUS, SPLAT_FORCE, SHADING, COLOR_UPDATE_SPEED, BACK_COLOR, PAUSED: false };
    let pointers: PointerInstance[] = [new PointerInstance()];
    let gl: WebGLRenderingContext | WebGL2RenderingContext;
    let ext: WebGLExtensions;
    const glContextResult = getWebGLContext(canvas);
    if (!glContextResult) { console.error("Failed to get WebGL context."); return; }
    gl = glContextResult.gl;
    ext = glContextResult.ext;
    if (!ext.supportLinearFiltering) { config.DYE_RESOLUTION = 256; config.SHADING = false; }
    function getWebGLContext(canvas: HTMLCanvasElement): { gl: WebGLRenderingContext | WebGL2RenderingContext; ext: WebGLExtensions } | null {
      const params = { alpha: true, depth: false, stencil: false, antialias: false, preserveDrawingBuffer: false };
      let tempGl: WebGLRenderingContext | WebGL2RenderingContext | null = canvas.getContext("webgl2", params) as WebGL2RenderingContext | null;
      const isWebGL2 = !!tempGl;
      if (!isWebGL2) { tempGl = (canvas.getContext("webgl", params) || canvas.getContext("experimental-webgl", params)) as WebGLRenderingContext | null; }
      ...
      if (isWebGL2) {
        ...
        if (halfFloat) { halfFloatTexType = halfFloat.HALF_FLOAT_OES; }
      }
      ...
      return { gl, ext: { formatRGBA, formatRG, formatR, halfFloatTexType, supportLinearFiltering: supportLinearFilteringExt, }, };
    }
    function getSupportedFormat(gl: WebGLRenderingContext | WebGL2RenderingContext, internalFormat: number, format: number, type: number | undefined): SupportedFormat | null {
      if (!type) return null;
    ...
    class Material {
      vertexShader: WebGLShader; fragmentShaderSource: string; programs: Record<number, WebGLProgram | undefined>; activeProgram: WebGLProgram | null; uniforms: Record<string, WebGLUniformLocation | null>;
      constructor(vertexShader: WebGLShader, fragmentShaderSource: string) { this.vertexShader = vertexShader; this.fragmentShaderSource = fragmentShaderSource; this.programs = {}; this.activeProgram = null; this.uniforms = {}; }
      setKeywords(keywords: string[]) {
        let hash = 0; for (let i = 0; i < keywords.length; i++) hash += hashCode(keywords[i]);
        let program = this.programs[hash];
        if (program == null) {
          const fragmentShader = compileShader(gl.FRAGMENT_SHADER, this.fragmentShaderSource, keywords);
          if (!fragmentShader) { console.error("Failed to compile fragment shader for material"); return; }
          const newProgram = createProgram(this.vertexShader, fragmentShader);
          if (!newProgram) { console.error("Failed to create program for material"); return; }
          program = newProgram; this.programs[hash] = program;
        }
    ...
    class Program {
      ...
      constructor(vertexShader: WebGLShader | null, fragmentShader: WebGLShader | null) {
        if (!vertexShader || !fragmentShader) { throw new Error("Vertex or Fragment shader is null for Program"); }
        const program = createProgram(vertexShader, fragmentShader);
        if (!program) { throw new Error("Failed to create program in Program constructor"); }
        this.program = program; this.uniforms = getUniforms(this.program);
      }
      bind() { gl.useProgram(this.program); }
    }
    function createProgram(vertexShader: WebGLShader, fragmentShader: WebGLShader): WebGLProgram | null {
      const program = gl.createProgram(); if (!program) { console.error("Failed to create GL program object."); return null; }
      gl.attachShader(program, vertexShader); gl.attachShader(program, fragmentShader); gl.linkProgram(program);
      if (!gl.getProgramParameter(program, gl.LINK_STATUS)) { console.trace(gl.getProgramInfoLog(program)); gl.deleteProgram(program); return null; }
      return program;
    }
    ...
    function compileShader(type: number, source: string, keywords?: string[]): WebGLShader | null {
      source = addKeywords(source, keywords); const shader = gl.createShader(type); if (!shader) { console.error("Failed to create GL shader object."); return null; }
      gl.shaderSource(shader, source); gl.compileShader(shader);
      if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) { console.trace(gl.getShaderInfoLog(shader)); gl.deleteShader(shader); return null; }
      return shader;
    }
    function addKeywords(source: string, keywords?: string[]): string {
      if (!keywords || keywords.length === 0) return source;
      let keywordsString = ""; keywords.forEach((keyword) => { keywordsString += "#define " + keyword + "\n"; });
    ...
    if (!baseVertexShader) { console.error("Base vertex shader failed to compile"); return; }
    // IMPORTANT: Remplacez "..." par le code complet de vos shaders GLSL
    ...
    if (!copyShader || !clearShader || !splatShader || !advectionShader || !divergenceShader || !curlShader || !vorticityShader || !pressureShader || !gradientSubtractShader) { console.error("One or more fragment shaders failed to compile."); return; }
    const blit = (() => {
    ...
    let dye: DoubleFBO | null = null; let velocity: DoubleFBO | null = null; let divergence: FBO | null = null; let curl: FBO | null = null; let pressure: DoubleFBO | null = null;
    ...
    function initFramebuffers() {
      ...
      const texType = ext.halfFloatTexType; if (texType === undefined) { console.error("Half float texture type not supported."); return; }
      if (!ext.formatRGBA || !ext.formatRG || !ext.formatR) { console.error("Required texture formats not supported."); return; }
    ...
    function createFBO(w: number, h: number, internalFormat: number, format: number, type: number, param: number): FBO {
      ...
      return { texture, fbo, width: w, height: h, texelSizeX: 1.0 / w, texelSizeY: 1.0 / h, attach(id: number) { gl.activeTexture(gl.TEXTURE0 + id); gl.bindTexture(gl.TEXTURE_2D, this.texture); return id; }, };
    }
    ...
    function resizeDoubleFBO(target: DoubleFBO, w: number, h: number, internalFormat: number, format: number, type: number, param: number): DoubleFBO {
      ...
      if (target.read.texture) gl.deleteTexture(target.read.texture); if (target.read.fbo) gl.deleteFramebuffer(target.read.fbo);
      if (target.write.texture) gl.deleteTexture(target.write.texture); if (target.write.fbo) gl.deleteFramebuffer(target.write.fbo);
    ...
    let lastUpdateTime = Date.now(); let colorUpdateTimer = 0.0; let animationFrameId: number | null = null;
    function mainLoop() {
      if (!isActive) return;
      const dt = calcDeltaTime(); if (resizeCanvas()) initFramebuffers();
      updateColors(dt); applyInputs();
      if (dye && velocity && divergence && curl && pressure) { step(dt); render(null); }
      animationFrameId = requestAnimationFrame(mainLoop);
    }
    function calcDeltaTime(): number { const now = Date.now(); let dt = (now - lastUpdateTime) / 1000; dt = Math.min(dt, 0.016666); lastUpdateTime = now; return dt; }
    function resizeCanvas(): boolean { const width = scaleByPixelRatio(canvas.clientWidth); const height = scaleByPixelRatio(canvas.clientHeight); if (canvas.width !== width || canvas.height !== height) { canvas.width = width; canvas.height = height; return true; } return false; }
    ...
    function step(dt: number) {
      if (config.PAUSED || !dye || !velocity || !divergence || !curl || !pressure) return;
      gl.disable(gl.BLEND);
    ...
    function render(target: FBO | null) {
      if (config.PAUSED || !dye) return;
      gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA); gl.enable(gl.BLEND);
      drawDisplay(target);
    }
    function drawDisplay(target: FBO | null) {
      if (!dye) return; const width = target == null ? gl.drawingBufferWidth : target.width; const height = target == null ? gl.drawingBufferHeight : target.height;
    ...
    function splatPointer(pointer: PointerInstance) { const dx = pointer.deltaX * config.SPLAT_FORCE; const dy = pointer.deltaY * config.SPLAT_FORCE; splat(pointer.texcoordX, pointer.texcoordY, dx, dy, pointer.color); }
    function clickSplat(pointer: PointerInstance) { const color = generateColor(); color.r *= 10.0; color.g *= 10.0; color.b *= 10.0; const dx = 1000 * (Math.random() - 0.5); const dy = 1000 * (Math.random() - 0.5); splat(pointer.texcoordX, pointer.texcoordY, dx, dy, color); }
    function splat(x: number, y: number, dx: number, dy: number, color: ColorRGB) {
      if (config.PAUSED || !velocity || !dye) return;
    ...
    function correctRadius(radius: number): number { const aspectRatio = canvas.width / canvas.height; if (aspectRatio > 1) radius *= aspectRatio; return radius; }
    function updatePointerDownData(pointer: PointerInstance, id: number, posX: number, posY: number) { pointer.id = id; pointer.down = true; pointer.moved = false; pointer.texcoordX = posX / canvas.width; pointer.texcoordY = 1.0 - posY / canvas.height; pointer.prevTexcoordX = pointer.texcoordX; pointer.prevTexcoordY = pointer.texcoordY; pointer.deltaX = 0; pointer.deltaY = 0; pointer.color = generateColor(); }
    function updatePointerMoveData(pointer: PointerInstance, posX: number, posY: number) { pointer.prevTexcoordX = pointer.texcoordX; pointer.prevTexcoordY = pointer.texcoordY; pointer.texcoordX = posX / canvas.width; pointer.texcoordY = 1.0 - posY / canvas.height; pointer.deltaX = correctDeltaX(pointer.texcoordX - pointer.prevTexcoordX); pointer.deltaY = correctDeltaY(pointer.texcoordY - pointer.prevTexcoordY); pointer.moved = Math.abs(pointer.deltaX) > 0 || Math.abs(pointer.deltaY) > 0; }
    function updatePointerUpData(pointer: PointerInstance) { pointer.down = false; }
    function correctDeltaX(delta: number): number { const aspectRatio = canvas.width / canvas.height; if (aspectRatio < 1) delta *= aspectRatio; return delta; }
    function correctDeltaY(delta: number): number { const aspectRatio = canvas.width / canvas.height; if (aspectRatio > 1) delta /= aspectRatio; return delta; }
    function generateColor(): ColorRGB { const c = HSVtoRGB(Math.random(), 1.0, 1.0); c.r *= 0.15; c.g *= 0.15; c.b *= 0.15; return c; }
    ...
    function wrap(value: number, min: number, max: number): number { const range = max - min; if (range === 0) return min; return ((value - min) % range) + min; }
    function getResolution(resolution: number): { width: number; height: number } { let aspectRatio = gl.drawingBufferWidth / gl.drawingBufferHeight; if (aspectRatio < 1) aspectRatio = 1.0 / aspectRatio; const min = Math.round(resolution); const max = Math.round(resolution * aspectRatio); if (gl.drawingBufferWidth > gl.drawingBufferHeight) return { width: max, height: min }; else return { width: min, height: max }; }
    function scaleByPixelRatio(input: number): number { const pixelRatio = window.devicePixelRatio || 1; return Math.floor(input * pixelRatio); }
    function hashCode(s: string): number { if (s.length === 0) return 0; let hash = 0; for (let i = 0; i < s.length; i++) { hash = (hash << 5) - hash + s.charCodeAt(i); hash |= 0; } return hash; }
    let firstInteractionDone = false;
    const handleMouseDown = (e: MouseEvent) => {
      if (!firstInteractionDone) { firstInteractionDone = true; if (animationFrameId === null) animationFrameId = requestAnimationFrame(mainLoop); }
      const pointer = pointers[0]; const posX = scaleByPixelRatio(e.clientX); const posY = scaleByPixelRatio(e.clientY);
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      if (touches.length > 0) { const touch = touches[0]; const posX = scaleByPixelRatio(touch.clientX); const posY = scaleByPixelRatio(touch.clientY); updatePointerDownData(pointer, touch.identifier, posX, posY); clickSplat(pointer); }
    };
    const handleTouchMove = (e: TouchEvent) => {
      const touches = e.targetTouches; const pointer = pointers[0];
      if (touches.length > 0) { const touch = touches[0]; const posX = scaleByPixelRatio(touch.clientX); const posY = scaleByPixelRatio(touch.clientY); updatePointerMoveData(pointer, posX, posY); }
    };
    const handleTouchEnd = (e: TouchEvent) => { const pointer = pointers[0]; updatePointerUpData(pointer); };
    animationFrameId = requestAnimationFrame(mainLoop); // Start loop immediately
    ...
    return () => {
      isActive = false; if (animationFrameId !== null) cancelAnimationFrame(animationFrameId);
      window.removeEventListener("mousedown", handleMouseDown);
      window.removeEventListener("mousemove", handleMouseMove);
      window.removeEventListener("touchstart", handleTouchStart);
      window.removeEventListener("touchmove", handleTouchMove);
      window.removeEventListener("touchend", handleTouchEnd);
      const fbosToClean: (FBO | DoubleFBO | null)[] = [dye, velocity, divergence, curl, pressure];
      fbosToClean.forEach(fboItem => {
        if (fboItem) {
          if ('read' in fboItem && 'write' in fboItem) {
            if (fboItem.read.texture) gl.deleteTexture(fboItem.read.texture); if (fboItem.read.fbo) gl.deleteFramebuffer(fboItem.read.fbo);
            if (fboItem.write.texture) gl.deleteTexture(fboItem.write.texture); if (fboItem.write.fbo) gl.deleteFramebuffer(fboItem.write.fbo);
          } else { if (fboItem.texture) gl.deleteTexture(fboItem.texture); if (fboItem.fbo) gl.deleteFramebuffer(fboItem.fbo); }
        }
      });
      // Consider deleting shaders and programs as well for complete cleanup
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [SIM_RESOLUTION, DYE_RESOLUTION, DENSITY_DISSIPATION, VELOCITY_DISSIPATION, PRESSURE, PRESSURE_ITERATIONS, CURL, SPLAT_RADIUS, SPLAT_FORCE, SHADING, COLOR_UPDATE_SPEED, BACK_COLOR,]);
  return (
