...
// This script applies the fix using the Supabase API

require('dotenv').config();
const fetch = require('node-fetch');

// Get Supabase project ID and service key from environment variables
const SUPABASE_PROJECT_ID = process.env.SUPABASE_PROJECT_ID || 'auzkjkliwlycclkpjlbl';
...
async function applyMigration() {
  ...
  try {
    const migrationName = 'fix_plan_type_enum_issue_complete';
    const migrationSQL = `
    ...
    const response = await fetch(
      `https://api.supabase.com/v1/projects/${SUPABASE_PROJECT_ID}/sql`,
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      throw new Error(`Failed to apply migration: ${JSON.stringify(result)}`);
    }
