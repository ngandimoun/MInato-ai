...
async function fixEvasionPolicy() {
  try {
    console.log('🔧 Applying evasion tables policy fix...')
    
    const supabase = await createServerSupabaseClient()
    
    // SQL to fix the infinite recursion issue in evasion_room_participants RLS policy
    const migrationSQL = `
      -- ============================================================================
    ...
    console.log('✅ Evasion tables policy fixed successfully!')
    
  } catch (error) {
    console.error('❌ Failed to apply evasion policy fix:', error)
    process.exit(1)
  }
}
