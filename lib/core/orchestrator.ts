...
export class Orchestrator {
  ...
  public async runOrchestration(
    ...
    try {
        ...
        toolRouterPrompt, textQueryForRouter,
        {
          ...
          items: {
            ...
            properties: {
              ...
              arguments: { type: "object", additionalProperties: false }, // Ensure arguments object is also strict
              reason: { type: "string" } // Add reason here
            },
            required: ["tool_name", "arguments", "reason"] // Ensure 'reason' is in required
          }
        },
        "minato_tool_router_v1", history.filter(m => typeof m.content === 'string'),
      ...
      if (finalStructuredResult && (finalStructuredResult as any).title) entitiesForMemorySearch.push((finalStructuredResult as any).title);
      ...
      return {
        ...
        transcription: typeof userInput === 'string' && apiContext?.transcription ? apiContext.transcription : (textQueryForRouter !== userInput ? textQueryForRouter : null),
        llmUsage: llmUsage_total,
        attachments: [],
      };
  ...
  async processAudioMessage(
    ...
    const userName = apiContext?.userName || await this.getUserFirstName(userId) || DEFAULT_USER_NAME;
    try {
      ...
      if (!fetchResponse.ok) {
        logger.error(`--- ${turnIdentifier} Audio fetch failed: ${fetchResponse.status} ${fetchResponse.statusText} from URL: ${audioSignedUrl.substring(0, 100)}...`);
        throw new Error(`Audio fetch failed: ${fetchResponse.status} ${fetchResponse.statusText}`);
      }
      ...
      if (orchResult && orchResult.error && !orchResult.clarificationQuestion) {
        // This error is from runOrchestration itself, not an API error to re-throw for TTS
        // The user-facing error will be part of orchResult.response if set, or orchResult.error
      ...
      const userFacingError = `Failed processing audio for ${userName}. Details: ${errorMessageString.substring(0, 100)}`;
