...
try {
  ...
  // Corrected schema for minato_tool_router_v1 for generateStructuredJson
  const routerSchema = { 
    type: "array" as const, 
    items: { 
        ...
        properties: { 
            ...
            arguments: {type: "object" as const, additionalProperties: true }, // Allow dynamic properties
            reason: {type: "string" as const}
        }, 
        required: ["tool_name", "arguments", "reason"],
        additionalProperties: false // Item object itself is strict
    } 
  };
  ...
  if ("error" in routerResult) {
      ...
      routedTools = routerResult; // routerResult should be ToolRouterPlan (array of steps)
