...
if ("error" in llmChoiceResult || !llmChoiceResult || !llmChoiceResult.action_type) { 
  ...
  logger.error(`${logPrefix} LLM planner failed: ${errorMsg}. Raw: ${JSON.stringify(llmChoiceResult).substring(0, 500)}`); 
...
if (plan && plan.steps) {
    plan.steps = plan.steps.filter(step => step && typeof step.type === 'string' && typeof step.description === 'string' && (step.type === 'tool_call' || step.type === 'llm_process' ? typeof (step as any).outputVar === 'string' : true));
    if (plan.steps.some(step => step.type === 'tool_call' && (!step.toolName || typeof step.toolName !== 'string'))) {
        logger.error(`${logPrefix} Planner returned a tool_call step without a valid toolName.`);
...
if (plan && Array.isArray(plan.steps) && plan.steps.length > WORKFLOW_MAX_TOOLS_PER_TURN) { 
  logger.warn(`${logPrefix} LLM planner returned ${plan.steps.length} steps, exceeding max ${WORKFLOW_MAX_TOOLS_PER_TURN}. Truncating.`); 
  plan.steps = plan.steps.slice(0, WORKFLOW_MAX_TOOLS_PER_TURN); 
  if (!plan.is_partial_plan) (plan as DynamicWorkflowPlanWithPartial).is_partial_plan = true; 
  if (!plan.continuation_summary) (plan as DynamicWorkflowPlanWithPartial).continuation_summary = "Minato can explore this further if you'd like."; 
}
return {
    plan: plan, 
...
if (wfState.status === "waiting_for_user" && wfState.clarificationPending) {
    ...
    if (wfState.currentStepIndex < currentPlanSteps.length) {
        ...
        if (planResult.actionType === "generate_dynamic_workflow" && planResult.plan) {
            ...
            wfState.executedStepsHistory = (wfState.dynamicPlan?.steps || []).map(s => s.description || s.type); 
    ...
    if (parallelExecutionGroup.length > 0) {
        ...
        const toolPromises = parallelExecutionGroup.map(toolCallStep => {
            ...
            if (tool.name === "InternalTaskTool") {
                if (!actualToolArgs.action && toolCallStep.toolArgs?.action) {
                    ...
                    logger.warn(`${logPrefix} [Parallel Fallback] 'action' for InternalTaskTool was missing after substitution, taking from original plan's toolArgs: '${actualToolArgs.action}'`);
                } else if (!actualToolArgs.action) {
                    const errorMsg = `[Parallel CRITICAL] 'action' argument missing for InternalTaskTool in step '${toolCallStep.description}'. Cannot execute. Tool Args from Plan: ${JSON.stringify(toolCallStep.toolArgs)}`;
        ...
        try {
            ...
            if (err instanceof Error) {
                ...
                detailedErrorMessage = (err as any).message;
            } else if (err) {
            ...
            wfState.error = `Parallel tool error: ${detailedErrorMessage.substring(0, 250)}`;
            this.activeWorkflows.set(sessionId, wfState);
            responseIntentType = "error";
            return { 
                responseText: `Minato encountered an issue with a parallel task, ${userName}: ${detailedErrorMessage.substring(0, 150)}.`,
                structuredData: lastToolOutputData, 
                workflowFeedback: { 
                    workflowName: wfState.fullPlanGoal, 
                    currentStepDescription: `Error during parallel tasks: ${detailedErrorMessage.substring(0, 100)}`, 
                    status: "failed" 
                }, 
                error: wfState.error, 
                isComplete: true, 
        ...
        try {
            if (step.type === "tool_call") {
                ...
                if (tool.name === "InternalTaskTool") {
                    if (!actualToolArgs.action && toolCallStep.toolArgs?.action) {
                        actualToolArgs.action = toolCallStep.toolArgs.action;
                        logger.warn(`${logPrefix} [Single Fallback] 'action' for InternalTaskTool was missing after substitution, taking from original plan's toolArgs: '${actualToolArgs.action}'`);
                    } else if (!actualToolArgs.action) {
                         const errorMsg = `[Single CRITICAL] 'action' argument missing for InternalTaskTool in step '${toolCallStep.description}'. Cannot execute. Tool Args from Plan: ${JSON.stringify(toolCallStep.toolArgs)}`;
                         logger.error(logPrefix + " " + errorMsg);
