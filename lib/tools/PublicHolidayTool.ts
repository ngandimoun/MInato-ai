...
export class PublicHolidayTool extends BaseTool {
  ...
  private mapApiHoliday(apiHoliday: HolidaysTypes.Holiday): HolidayData {
    ...
    return {
      ...
      type: apiHoliday.type,
      rule: apiHoliday.rule,
    };
  }
  ...
  async execute(
    ...
    let outputStructuredData: PublicHolidayStructuredOutput = {
      ...
      query: input,
      location: countryCode?.toUpperCase() || "invalid",
      error: null,
      year: null,
      queryDate: null,
      isHoliday: null,
    ...
    if (!hd) {
      let errorMsg = `Could not get holiday data for country "${countryCode}". It might be unsupported.`;
      if (regionCode)
        errorMsg = `Region "${regionCode}" might be unsupported for country "${countryCode}", or the combination is invalid.`;
      outputStructuredData.error = errorMsg;
      return {
        error: errorMsg,
    ...
    try {
      if (date) {
        ...
        if (holidayResult && typeof holidayResult === "object") {
          const singleHoliday =
            holidayResult as unknown as HolidaysTypes.Holiday;
        ...
        const publicHolidays = holidays.filter(
          (h: HolidaysTypes.Holiday) => h.type === "public"
        );
        if (!Array.isArray(publicHolidays) || publicHolidays.length === 0) {
          const resultStr = `No public holidays found for ${locationName} in ${targetYear}.`;
        ...
        const listString =
          outputStructuredData.holidays
            ?.slice(0, limit)
            .map((h: { date: any; name: any }) => `- ${h.date}: ${h.name}`)
            .join("\n") || "";
        const resultString = `Found ${
          publicHolidays.length
        } public holiday(s) for ${locationName} in ${targetYear}. ${
          publicHolidays.length > limit ? `The first ${limit}` : "They"
        } are:\n${listString}${
          publicHolidays.length > limit ? "\n(...and more)" : ""
        }`;
        return { result: resultString, structuredData: outputStructuredData };
      }
    } catch (error: any) {
      ...
      return {
        error: errorMsg,
        result: `Sorry, an error occurred while getting holiday information for ${locationName}.`,
        structuredData: outputStructuredData,
      };
    }
  }
