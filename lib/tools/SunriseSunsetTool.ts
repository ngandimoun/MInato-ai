...
export class SunriseSunsetTool extends BaseTool {
  ...
  argsSchema = {
    ...
    properties: {
      location: { type: ["string", "null"], description: "The city name or address (e.g., 'Paris, France', 'Tokyo'). If omitted, uses user's context location." },
      latitude: { type: ["number", "null"], description: "Optional explicit latitude. Overrides location name and context." },
      longitude: { type: ["number", "null"], description: "Optional explicit longitude. Overrides location name and context." },
      date: { type: ["string", "null"], format: "date", description: "Optional date to check (YYYY-MM-DD). Defaults to today." },
    },
  ...
  private async geocodeLocation(location: string, abortSignal?: AbortSignal): Promise<GeocodeResult> {
    ...
    try {
      ...
      if (!Array.isArray(data) || data.length === 0 || !data[0].lat || !data[0].lon) {
        this.log("warn", `Nominatim could not find coordinates for "${location}". Response:`, data);
  ...
  private getSafeTimezone(inputTz?: string | null): string {
    ...
    try { const found = findTimeZone(inputTz); if (found) return found.name; } catch { /* ignore */ }
    this.log("warn", `Could not validate provided timezone "${inputTz}", defaulting to UTC.`);
    return "Etc/UTC";
  }

  private formatTimeInZone(date: Date | undefined | null, timeZone: string): string | null {
    if (!date || isNaN(date.getTime())) return null;
    try { return formatInTimeZone(date, timeZone, "HH:mm"); }
  ...
  async execute(input: SunTimeInput, abortSignal?: AbortSignal): Promise<ToolOutput> {
    ...
    let locationName = input.location ?? undefined; // Use undefined if null for cleaner checks
    ...
    let structuredOutput: SunriseSunsetStructuredOutput = {
      ...
      resolvedLat: null, resolvedLon: null, resolvedLocation: locationName || "unknown",
    ...
    if (typeof lat === "number" && typeof lon === "number") {
      ...
      this.log("info", `No coordinates provided for ${input.context?.userName || "user"}, attempting to geocode "${locationName}"...`);
      const geoResult = await this.geocodeLocation(locationName, abortSignal);
      if (abortSignal?.aborted) { return { error: "Sun times check cancelled.", result: "Cancelled." }; }
      if (geoResult.error || geoResult.latitude === undefined || geoResult.longitude === undefined) {
        geoError = geoResult.error || `Could not find coordinates for "${locationName}".`;
        coordSource = "error";
      } else {
        lat = geoResult.latitude; lon = geoResult.longitude; coordSource = "geocoded_name";
        displayLocation = geoResult.displayName || locationName;
        this.log("info", `Using geocoded coords for "${locationName}": ${lat}, ${lon}`);
      }
    } else {
      coordSource = "error";
      geoError = "Missing location information. Provide coordinates or a location name.";
    }
    ...
    structuredOutput.resolvedLocation = displayLocation || locationName || "unknown";

    if (coordSource === "error" || typeof lat !== "number" || typeof lon !== "number") {
      const errorMsg = geoError || "Could not determine coordinates for the location.";
      structuredOutput.error = errorMsg;
      return { error: errorMsg, result: `Sorry, ${input.context?.userName || "User"}, Minato needs a valid location to check sun times. ${errorMsg}`, structuredData: structuredOutput };
    }
    ...
    try {
      ...
      structuredOutput.error = errorMsg;
      return { error: errorMsg, result: `Please provide the date in YYYY-MM-DD format for Minato, ${input.context?.userName || "User"}.`, structuredData: structuredOutput };
    }
    ...
    try {
      this.log("info", `Calculating sun times using SunCalc for ${lat.toFixed(4)},${lon.toFixed(4)} on ${dateString}`);
      ...
      if (!structuredOutput.sunriseLocal || !structuredOutput.sunsetLocal) {
        this.log("warn", `Failed to format sunrise/sunset times in timezone: ${timezone}`);
      ...
      return { error: structuredOutput.error, result: `Sorry, ${input.context?.userName || "User"}, there was an error calculating the sun times for that location.`, structuredData: structuredOutput };
    }
  }
}