...
interface OWMSys { type?: number; id?: number; country?: string; sunrise?: number; sunset?: number; }
interface OWMCoord { lon: number; lat: number; }
interface OWMResponse { coord: OWMCoord; weather: OWMWeather[]; base: string; main: OWMMain; visibility?: number; wind: OWMWind; clouds?: { all: number }; rain?: { "1h"?: number; "3h"?: number }; snow?: { "1h"?: number; "3h"?: number }; dt: number; sys: OWMSys; timezone: number; id: number; name: string; cod: number | string; message?: string; }
...
export class WeatherTool extends BaseTool {
  ...
  constructor() {
    ...
    if (!this.API_KEY) this.log("error", "OpenWeatherMap API Key (OPENWEATHERMAP_API_KEY) is missing. Tool will fail.");
    if (this.USER_AGENT.includes("support@example.com")) {
        this.log("warn", "Update WeatherTool USER_AGENT contact info with actual details.");
    }
  }

  private celsiusToFahrenheit(celsius: number): number { return (celsius * 9) / 5 + 32; }
  private mpsToKph(mps: number): number { return mps * 3.6; }
  private mpsToMph(mps: number): number { return mps * 2.23694; }
  ...
  async execute(input: WeatherInput, abortSignal?: AbortSignal): Promise<ToolOutput> {
    ...
    this.log("info", `${logPrefix} Fetching weather (requesting metric)... URL: ${url.replace(this.API_KEY, "***")}`);

    let finalStructuredData: CachedSingleWeather = {
      result_type: "weather", source_api: "openweathermap", query: queryInputForStructuredData, weather: null, error: undefined,
    };
    ...
    try {
      ...
      if (!response.ok || String(data.cod) !== "200") {
        ...
        let userMessage = `Sorry, ${input.context?.userName || "User"}, Minato couldn't find weather data for "${location}".`;
        if (String(data.cod) === "404") { errorDetail += ` Location not found: ${data.message || ""}`; userMessage = `Sorry, Minato couldn't find the location "${location}". Please check the name or zip code.`; }
        else if (String(data.cod) === "401") { errorDetail += ` Invalid API key: ${data.message || ""}`; userMessage = `Sorry, there's an issue with the weather service configuration. Minato will notify the team.`; }
        else { errorDetail += ` Message: ${data.message || "Unknown API error"}`; }
        this.log("error", `${logPrefix} ${errorDetail}`);
        finalStructuredData.error = errorDetail;
        return { error: errorDetail, result: userMessage, structuredData: finalStructuredData };
      }
      if (!data.main || !data.weather?.[0]) { throw new Error(`Weather API returned success code but missing core data (main/weather).`); }

      const tempC = data.main.temp; const feelsLikeC = data.main.feels_like;
      const tempF = this.celsiusToFahrenheit(tempC); const feelsLikeF = this.celsiusToFahrenheit(feelsLikeC);
      const description = data.weather[0].description; const humidity = data.main.humidity;
      ...
      let resultString = `Current weather in ${locationName} for ${input.context?.userName || "User"}: ${description}. `;
      if (displayUnits === "imperial") {
        resultString += `Temperature: ${Math.round(tempF)}°F (feels like ${Math.round(feelsLikeF)}°F). `;
        resultString += `Wind: ${windDir} at ${windSpeedMph} mph. Humidity: ${humidity}%.`;
      } else {
        resultString += `Temperature: ${tempC.toFixed(1)}°C (feels like ${feelsLikeC.toFixed(1)}°C). `;
