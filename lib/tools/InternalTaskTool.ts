...
action: "add_task" | "list_tasks" | "complete_task"; // Kept as non-optional
...
due_date: { type: ["string", "null"] as const, description: "Optional due date (YYYY-MM-DD format, e.g., '2024-08-15') when adding a task. Can be null." } as OpenAIToolParameterProperties,
limit: { type: ["number", "null"] as const, description: "Maximum number of tasks to list (must be between 1 and 20). If null or omitted for 'list_tasks', defaults to 10." } as OpenAIToolParameterProperties,
},
...
const { userId: contextUserId } = input; // Extract contextUserId from input directly
// --- FIX START: Validate action directly from input ---
// The 'action' property is crucial for the tool's logic.
// Even if the schema declares it as required, the LLM might occasionally fail to provide it,
// or it might be lost/mangled during argument substitution in the WorkflowEngine.
// This explicit check ensures the tool fails gracefully with a clear error.
const action = input.action;
if (!action || !["add_task", "list_tasks", "complete_task"].includes(action)) {
    const errorMsg = `Invalid or missing 'action' parameter for InternalTaskTool. Received: '${input.action}' (type: ${typeof input.action}). Input object keys: ${Object.keys(input).join(', ')}`;
    // Try to get userId from context if available for logging
    const loggableUserId = input.context?.userId || contextUserId || "UnknownUser";
    logger.error(`[InternalTaskTool User:${loggableUserId.substring(0,8)}] ${errorMsg}`, { fullInput: input });
    return {
        error: errorMsg,
        result: "Minato needs to know whether to add, list, or complete a task. The instruction was unclear.",
        structuredData: {
            result_type: "internal_tasks",
            source_api: "internal_memory",
            query: input, // Log the received input for debugging
            action: "error" as any, // Cast to 'any' as "error" is not in the action enum
            status: "error",
            tasks: [],
            error: errorMsg
        }
...
const effectiveFilter = (input.filter === null || input.filter === undefined || input.filter.trim() === "") ? "pending" : input.filter;
const due_date = (input.due_date === null || input.due_date === undefined || input.due_date.trim() === "") ? undefined : input.due_date;
...
const queryInputForStructuredData = {...input, action: action, filter: effectiveFilter, limit: effectiveLimit};
...
// Specific argument checks based on validated action
if (action === "add_task" && (!task_content || task_content.trim().length === 0)) {
  outputStructuredData.error = "Task content required for 'add_task'."; 
  return { error: outputStructuredData.error, result: `What is the task, ${input.context?.userName || "User"}?`, structuredData: outputStructuredData };
}
if (action === "complete_task" && (!task_id || typeof task_id !== "string" || task_id.trim().length === 0)) {
  outputStructuredData.error = "Task ID required for 'complete_task'."; 
  return { error: outputStructuredData.error, result: `Which task ID should Minato complete, ${input.context?.userName || "User"}?`, structuredData: outputStructuredData };
}

try {
  if (action === "add_task") {
    // task_content is now guaranteed to be a non-empty string here due to the check above
    ...
    const memoryToAdd: StoredMemoryUnit = {
      ...
      role: "system", content: task_content!.trim(), // Safe to use ! due to check
    ...
    // task_id is now guaranteed to be a non-empty string here
  ...
  // Since action is validated at the start, an 'else' case for invalid action is theoretically unreachable.
  // If it were, TypeScript would complain about 'action' possibly being of type 'never'.
} catch (error: any) {
  ...
  // Ajout d'un return impossible pour satisfaire TypeScript
  return { error: "Internal error: unreachable code path in InternalTaskTool.execute.", result: "Internal error.", structuredData: outputStructuredData };
}
}