...
// Helper functions (getGoogleRefreshToken, getGoogleAuthClient, handleGoogleAuthFailure)
...
async function getGoogleRefreshToken(userId: string): Promise<string | null> {
  ...
  try {
    ...
    if (error && error.code !== "PGRST116") { logger.error(`${logPrefix} Supabase error:`, error); return null; }
...
function getGoogleAuthClient(refreshToken: string): OAuth2Client {
  ...
  oauth2Client.on("tokens", (tokens) => { if (tokens.refresh_token) logger.info("[getGoogleAuthClient] New refresh token received."); if (tokens.access_token) logger.debug(`[getGoogleAuthClient] Access token refreshed.`); });
  return oauth2Client;
}
...
function extractTextFromPayload(payload: gmail_v1.Schema$MessagePart | undefined): string {
  ...
  if (payload.mimeType === "text/plain") {
    ...
    for (const part of payload.parts) {
      ...
      if (!bodyText && part.mimeType === "multipart/alternative" && part.parts) {
        for (const subPart of part.parts) { if (subPart.mimeType === "text/plain") { bodyText = decodePayloadData(subPart.body?.data); if (bodyText) break; } }
        if (bodyText) break;
      }
    }
  }
  if (!bodyText && payload.mimeType === "text/html") {
    bodyText = decodePayloadData(payload.body?.data);
    bodyText = bodyText.replace(/<style[^>]*>.*?<\/style>/gs, "").replace(/<script[^>]*>.*?<\/script>/gs, "").replace(/<[^>]+>/g, " ").replace(/<!--.*?-->/gs, "").replace(/\s+/g, " ").trim();
  }
...
export class GoogleGmailReaderTool extends BaseTool {
  ...
  argsSchema = {
    ...
    properties: {
      action: { type: ["string", "null"], enum: ["get_recent_emails"], description: "Action to perform (default 'get_recent_emails').", default: "get_recent_emails" },
      maxResults: { type: ["number", "null"], minimum: 1, maximum: 10, description: "Max emails metadata (default 5).", default: 5 },
      query: { type: ["string", "null"], description: "Gmail search query (e.g., 'is:unread label:work'). Defaults to 'is:unread category:primary'.", default: "is:unread category:primary" },
      summarize_body: { type: ["boolean", "null"], description: "Summarize recent email bodies (default false). Needs more permissions.", default: false },
      summarize_limit: { type: ["number", "null"], minimum: 1, maximum: 3, description: "Max bodies to summarize if enabled (default 1).", default: 1 },
    },
  ...
  async execute(input: GmailInput, abortSignal?: AbortSignal): Promise<ToolOutput> {
    const { userId: contextUserId, maxResults, query, summarize_body, summarize_limit } = input;
    const userId = input.context?.userId || contextUserId;
    const effectiveMaxResults = maxResults ?? 5;
    const effectiveQuery = (query === null || query === undefined) ? "is:unread category:primary" : query;
    const effectiveSummarizeBody = summarize_body ?? false;
    const effectiveSummarizeLimit = summarize_limit ?? 1;

    const logPrefix = `[GoogleGmailReaderTool User:${userId?.substring(0,8)} Query:"${effectiveQuery.substring(0,20)}"]`;
    const queryInputForStructuredData = { ...input, maxResults: effectiveMaxResults, query: effectiveQuery, summarize_body: effectiveSummarizeBody, summarize_limit: effectiveSummarizeLimit };

    if (abortSignal?.aborted) { return { error: "Email check cancelled.", result: "Cancelled." }; }
    if (!userId) return { error: "User ID missing.", result: `I need to know who you are, ${input.context?.userName || "User"}, to check your email.`, structuredData: { result_type: "email_headers", source_api: "google_gmail", query: queryInputForStructuredData, emails: [], error: "User ID missing" } };
    ...
    if (!userState?.googleemailenabled) {
      ...
      return {
        ...
        result: `Access to Google Email is not enabled in your settings, ${input.context?.userName || "User"}. Please enable it if you want me to check your emails.`,
        structuredData: { result_type: "permission_denied", source_api: "google_gmail", query: queryInputForStructuredData, message:"Google Email access not enabled by user.", error: "Permission denied by user settings" },
      };
    }
    ...
    try {
      ...
      if (!refreshToken) {
        ...
        return { error: "Gmail not connected.", result: `Please connect your Gmail account in settings first, ${input.context?.userName || "User"}.`, structuredData: { result_type: "email_headers", source_api: "google_gmail", query: queryInputForStructuredData, emails: [], error: "Gmail not connected" } };
      }
      auth = getGoogleAuthClient(refreshToken);
    } catch (authError: any) {
      logger.error(`${logPrefix} Authentication setup failed:`, authError.message);
      return { error: `Authentication setup failed: ${authError.message}`, result: `Sorry, ${input.context?.userName || "User"}, there was a problem setting up email access.`, structuredData: { result_type: "email_headers", source_api: "google_gmail", query: queryInputForStructuredData, emails: [], error: `Auth setup failed: ${authError.message}` } };
    }
    ...
    try {
      ...
      if (!messages || messages.length === 0) {
        ...
        return { result: `No ${queryDesc} found for ${input.context?.userName || "User"} right now.`, structuredData: outputStructuredData };
      }
      ...
      const detailResults = settledDetailResults
        ...
        .map(res => (res as PromiseFulfilledResult<gmail_v1.Schema$Message | null>).value)
        .filter((meta): meta is gmail_v1.Schema$Message => meta !== null && !!meta.id);

      if (detailResults.length === 0) {
        logger.warn(`${logPrefix} Found message IDs but failed to fetch details for any (or aborted).`);
        return { result: `Found some message IDs for ${input.context?.userName || "User"} but couldn't retrieve their details.`, structuredData: outputStructuredData };
      }
      ...
      for (let i = 0; i < detailResults.length; i++) {
          ...
          if (effectiveSummarizeBody && i < effectiveSummarizeLimit) {
              ...
              if (bodyText) {
                  ...
                  logger.warn(`${logPrefix} Could not extract text body for summarization from msg ${meta.id}.`);
                  const targetEmail = formattedEmails.find(e => e.id === meta.id);
      ...
      if (summaryPromises.length > 0) {
        logger.debug(`${logPrefix} Waiting for ${summaryPromises.length} body summaries...`);
      ...
      if (error.name === 'AbortError') { outputStructuredData.error = "Request timed out."; return { error: "Email check timed out.", result: `Sorry, ${input.context?.userName || "User"}, checking email took too long.`, structuredData: outputStructuredData };}
      if (statusCode === 401 || (statusCode === 403 && (apiErrorMessage?.includes("invalid_grant") || apiErrorMessage?.includes("access_denied")))) {
        ...
        return { error: outputStructuredData.error, result: `Minato couldn't access your Gmail, ${input.context?.userName || "User"}. Please try reconnecting it in settings.`, structuredData: outputStructuredData };
        ...
        return { error: outputStructuredData.error, result: `It seems Minato doesn't have permission to access your emails anymore, ${input.context?.userName || "User"}. Please check connection settings.`, structuredData: outputStructuredData };
      }
      return { error: outputStructuredData.error, result: `Sorry, ${input.context?.userName || "User"}, there was an error accessing your email.`, structuredData: outputStructuredData };
    }
  }
}