...
function getToolDescriptionsForPlanner(toolRegistry: { [key: string]: BaseTool }): string {
    return Object.values(toolRegistry)
        ...
        .map(tool => {
            // Corrected template literal for string formatting
...
Instructions:
Based on the User Query and context, identify the primary intent(s).
If tools can help fulfill the intent, select up to ${MAX_TOOLS_PER_TURN} tools from the "Available Tools" list.
For each selected tool, determine the necessary arguments with high confidence based on the provided information. If an argument cannot be confidently inferred, it's often better to let the main response model (GPT-4o) handle follow-up or use broader tool capabilities.
Provide a brief reason for selecting each tool.
If no tools are immediately necessary for this specific query (e.g., it's simple chitchat, a direct question Minato can answer, or a follow-up that doesn't require new actions), output an empty array.
...
Vocal Delivery Style: Consider the desired vocal delivery for your response (e.g., tone, pace, emphasis). Your choice of 'intentType' in the output JSON will guide the Text-to-Speech engine. Do not include SSML-like tags (e.g., <break>, <emphasis>) directly in the 'responseText'.
For example, if you want a pause, choose an intentType and craft your text to naturally flow with pauses. If you want emphasis, the TTS engine will interpret the chosen 'intentType' and apply natural emphasis.
Example of how to think: For "That's a great question, {userName}. Let me think...", you might choose an intentType like 'thinking' or 'informative' which would imply certain pauses and emphasis, but the 'responseText' itself would just be "That's a great question, {userName}. Let me think...".
...
'''json
{
"responseText": "The natural language response for {userName}, from Minato. This text must be clean and ready for display to the user, without any SSML-like or XML tags. Vocal delivery instructions are handled separately via the 'intentType'. If a tool_router_follow_up_suggestion was provided, this responseText MUST include a question to {userName} about proceeding with it.",
