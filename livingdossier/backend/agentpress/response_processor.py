# livingdossier/backend/agentpress/response_processor.py
...
from typing import List, Dict, Any, Optional, AsyncGenerator, Tuple, Union, Callable, Literal, cast, Type
...
# Define base classes that will be used for type annotations
@dataclass
class ToolResult:
...
class ToolRegistry:
    ...
    def __init__(self):
        self.xml_tools: Dict[str, Any] = {}
    
    def get_xml_tool(self, name: str) -> Optional[Dict[str, Any]]:
        return self.xml_tools.get(name)
        
    def get_available_functions(self) -> Dict[str, Callable]:
        return {}

class XMLToolParser:
    """Parser for XML-based tool calls."""
...
# Create a langfuse stub class
class LangfuseStub:
    """Stub for langfuse service."""
    def trace(self, **kwargs: Any) -> StatefulTraceClient:
        return StatefulTraceClient()

# Set up default implementation
langfuse = LangfuseStub()

# Define helper functions with proper type annotations
def ensure_dict(obj: Any, default: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    if isinstance(obj, dict):
        return obj
    return {} if default is None else default
    
def ensure_list(obj: Any, default: Optional[List[Any]] = None) -> List[Any]:
...
def format_for_yield(obj: Dict[str, Any]) -> Dict[str, Any]:
    return obj

# Define a minimal token_counter function
...
class ResponseProcessor:
    ...
    async def _yield_message(self, message_obj: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Return empty dict if message_obj is None to satisfy return type
        return {}
        ...
        try:
            ...
            async for chunk in llm_response:
                ...
                if hasattr(chunk, 'choices') and chunk.choices:
                    ...
                    if delta and hasattr(delta, 'content') and delta.content:
                        ...
                        if not (config.max_xml_tool_calls > 0 and xml_tool_call_count >= config.max_xml_tool_calls):
                            ...
                            if self.trace:
                                self.trace.event(name="xml_tool_call_limit_reached", level="DEFAULT", 
                ...
                if finish_reason == "xml_tool_limit_reached":
                    ...
                    if self.trace:
                        self.trace.event(name="stopping_stream_processing_after_loop_due_to_xml_tool_call_limit", level="DEFAULT", 
                         ...
                         try:
                             if execution["task"].done():
                                 ...
                                 if tool_name in ['ask', 'complete']:
                                     ...
                                     if self.trace:
                                         self.trace.event(name="terminating_tool_completed_during_streaming", level="DEFAULT", 
                                                         status_message="Terminating tool '{tool_name}' completed during streaming. Setting termination flag.")
                                ...
                                if self.trace:
                                    self.trace.event(name="task_for_tool_index_not_done_after_wait", level="WARNING", 
                                                   status_message=f"Task for tool index {tool_idx} not done after wait.")
                         except Exception as e:
                             ...
                             if self.trace:
                                 self.trace.event(name="error_getting_result_for_pending_tool_execution", level="ERROR", 
                                                 status_message=f"Error getting result for pending tool execution {tool_idx}: {str(e)}")
                             context.error = e
                    ...
                    try:
                        ...
                        if self.trace:
                            self.trace.event(name="error_getting_result_yielding_status_for_pending_tool_execution", level="ERROR", 
                                           status_message=f"Error getting result/yielding status for pending tool execution {tool_idx}: {str(e)}")
                        context.error = e
            ...
            if finish_reason == "xml_tool_limit_reached":
                ...
                if self.trace:
                    self.trace.event(name="stream_finished_with_reason_xml_tool_limit_reached_after_xml_tool_calls", level="DEFAULT", 
                                    status_message=f"Stream finished with reason: xml_tool_limit_reached after {xml_tool_call_count} XML tool calls")
            ...
            if config.execute_tools:
                ...
                # Or execute now if not streamed
                    ...
                    if self.trace:
                        self.trace.event(name="executing_tools_after_stream", level="DEFAULT", 
            ...
            if agent_should_terminate:
                ...
                if last_assistant_message_object:
                    try:
                        ...
                        if self.trace:
                            self.trace.event(name="error_saving_assistant_response_end_for_stream_before_termination", level="ERROR", 
            ...
            if config.execute_tools and tool_calls_to_execute:
                ...
                if self.trace:
                    self.trace.event(name="executing_tools_with_strategy", level="DEFAULT", 
                                    status_message=f"Executing {len(tool_calls_to_execute)} tools with strategy: {config.tool_execution_strategy}")
    ...
    # XML parsing methods
    def _extract_tag_content(self, xml_chunk: str, tag_name: str) -> Tuple[Optional[str], str]:
        ...
        try:
            ...
            if self.trace:
                self.trace.event(name="error_extracting_tag_content", level="ERROR", 
    ...
    def _extract_attribute(self, opening_tag: str, attr_name: str) -> Optional[str]:
        ...
        try:
            ...
            if self.trace:
                self.trace.event(name="error_extracting_attribute", level="ERROR", 
    ...
    def _parse_xml_tool_call(self, xml_chunk: str) -> Optional[Tuple[Dict[str, Any], Dict[str, Any]]]:
        ...
        try:
            ...
            if not tag_match:
                ...
                if self.trace:
                    self.trace.event(name="no_tag_found_in_xml_chunk", level="ERROR", 
            ...
            if self.trace:
                self.trace.event(name="found_xml_tag", level="DEFAULT", 
                                status_message=f"Found XML tag: {xml_tag_name}")
            ...
            if not tool_info or not tool_info['schema'].xml_schema:
                ...
                if self.trace:
                    self.trace.event(name="no_tool_or_schema_found_for_tag", level="ERROR", 
            ...
            for mapping in schema.mappings:
                try:
                    if mapping.node_type == "attribute":
                        ...
                        remaining_chunk = new_remaining  # Update remaining_chunk with the new value
                    ...
                    if self.trace:
                        self.trace.event(name="error_processing_mapping", level="ERROR", 
    ...
    async def _execute_tool(self, tool_call: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # Ensure arguments is a dict for **kwargs unpacking
    ...
    async def _execute_tools_sequentially(self, tool_calls: List[Dict[str, Any]]) -> List[Tuple[Dict[str, Any], ToolResult]]:
        ...
        try:
            ...
            for index, tool_call in enumerate(tool_calls):
                ...
                try:
                    ...
                    if tool_name in ['ask', 'complete']:
                        ...
                        if self.trace:
                            self.trace.event(name="terminating_tool_executed", level="DEFAULT", 
            ...
            if self.trace:
                self.trace.event(name="sequential_execution_completed", level="DEFAULT", 
            ...
            completed_results: List[Tuple[Dict[str, Any], ToolResult]] = []
            ...
            error_results: List[Tuple[Dict[str, Any], ToolResult]] = [
        ...
        try:
            ...
            for i, (tool_call, result) in enumerate(zip(tool_calls, results)):
                if isinstance(result, Exception):
                    ...
                    if self.trace:
                        self.trace.event(name="error_executing_tool", level="ERROR", 
                                        status_message=f"Error executing tool {tool_call.get('function_name', 'unknown')}: {str(result)}")
                    # Create error result
    ...
    async def _add_tool_result(
        ...
        try:
            ...
            if parsing_details:
                ...
                if self.trace:
                    self.trace.event(name="adding_parsing_details_to_tool_result_metadata", level="DEFAULT", 
                                    status_message="Adding parsing_details to tool result metadata", metadata={"parsing_details": parsing_details})
            # ---
            ...
            if "id" in tool_call:
                ...
                if self.trace:
                    self.trace.event(name="adding_native_tool_result_for_tool_call_id", level="DEFAULT", 
            ...
            if is_mcp_tool:
                ...
                if self.trace:
                    self.trace.event(name="adding_mcp_tool_result_simplified", level="DEFAULT", 
                                    status_message="Adding MCP tool result with simplified format for LLM visibility")
            ...
            if self.trace:
                self.trace.event(name="error_adding_tool_result", level="ERROR", 
                                status_message=f"Error adding tool result: {str(e)}", metadata={"tool_call": tool_call, "result": result, "strategy": strategy, "assistant_message_id": assistant_message_id, "parsing_details": parsing_details})
            ...
            try:
                ...
                if self.trace:
                    self.trace.event(name="failed_even_with_fallback_message", level="ERROR", 
                                    status_message=f"Failed even with fallback message: {str(e2)}", metadata={"tool_call": tool_call, "result": result, "strategy": strategy, "assistant_message_id": assistant_message_id, "parsing_details": parsing_details})
    ...
    async def _yield_and_save_tool_completed(self, context: ToolExecutionContext, tool_message_id: Optional[str], thread_id: str, thread_run_id: str) -> Optional[Dict[str, Any]]:
        ...
        metadata: Dict[str, Any] = {"thread_run_id": thread_run_id}
        ...
        if context.function_name in ['ask', 'complete']:
            metadata["agent_should_terminate"] = "true"  # Use string instead of boolean
            logger.info(f"Marking tool status for '{context.function_name}' with termination signal.")
            if self.trace:
                self.trace.event(name="marking_tool_status_for_termination", level="DEFAULT", 
