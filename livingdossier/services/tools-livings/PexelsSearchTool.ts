//livingdossier/services/tools-livings/PexelsSearchTool.ts
...
// Local implementation of generateStructuredJson since the import is missing
async function generateStructuredJson<T>(
  ...
  try {
    // Simple implementation that returns an empty object
    // In a real implementation, this would call an LLM to generate structured data
    console.log(`[PexelsSearchTool] Using local generateStructuredJson implementation`);
    return {} as T;
  } catch (error) {
    console.error(`[PexelsSearchTool] Error in generateStructuredJson:`, error);
    return null;
  }
}
...
// Extended Config interface to include toolApiKeys
...
export class PexelsSearchTool extends BaseTool {
  ...
  constructor() {
    super();
    // Create a safe access to toolApiKeys which might not exist in appConfig
    const extendedConfig = appConfig as unknown as ExtendedConfig;
    const apiKey = extendedConfig.toolApiKeys?.pexels || process.env.PEXELS_API_KEY;
    if (!apiKey) {
  ...
  async execute(input: PexelsSearchInput, abortSignal?: AbortSignal): Promise<ToolOutput> {
    ...
    try {
      ...
      if ("error" in response) {
        ...
        // Create a safe access to toolApiKeys which might not exist in appConfig
        const extendedConfig = appConfig as unknown as ExtendedConfig;
        const apiKey = extendedConfig.toolApiKeys?.pexels || process.env.PEXELS_API_KEY || "DUMMY_PEXELS_KEY";
        const sanitizedError = typeof pexelsError.error === 'string' ? pexelsError.error.replace(apiKey, "***") : "Unknown Pexels API Error";
